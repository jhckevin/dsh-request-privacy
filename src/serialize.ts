...
// These names are owned by the stock agent runtime. Their descriptions are
// generated product metadata, not user-authored conversation content.
...
/** Remove product branding only from descriptions of stock runtime tools. */
export function neutralizePlatformToolDescription(name: string, description: string): string {
  if (!PLATFORM_TOOL_NAMES.has(name)) return description
  return description
    .replace(/DeepSeek Harness/gi, 'agent runtime')
    .replace(/\bHarness\b/gi, 'agent runtime')
}
