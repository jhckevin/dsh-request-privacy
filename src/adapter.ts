...
import { privacyHeaders } from './privacy.ts'
...
export interface DeepSeekConnectionOptions {
  /** Request metadata policy resolved with the same operation snapshot. */
  privacy: RequestPrivacyConfig
...
export interface DeepSeekAdapterOptions {
  ...
  /**
   * Owning Cordis plugin lifetime. Aborting it cancels every in-flight fetch
   * and stream so a hot unload does not retain sockets, readers, or watchdogs.
   */
...
export class DeepSeekAdapter extends LlmAdapter {
  ...
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    const lifecycleSignal = this.config.lifecycleSignal
    if (lifecycleSignal?.aborted) {
      throw new LlmError('DeepSeek adapter is unavailable because its plugin was unloaded', 'ABORTED')
    }
    const connection = this.config.options()
    const apiKey = await this.config.resolveApiKey(connection)
    if (lifecycleSignal?.aborted) {
      throw new LlmError('DeepSeek adapter was unloaded while resolving credentials', 'ABORTED')
    }
    ...
    const upstream = signals.length === 1 ? signals[0]! : AbortSignal.any(signals)
    ...
    try {
      ...
      if (lifecycleSignal?.aborted) {
        throw new LlmError('DeepSeek request aborted because its plugin was unloaded', 'ABORTED', { cause: error })
      }
  ...
  private async * request(
    ...
    const headers = {
      ...
      ...privacyHeaders(connection.privacy, {
