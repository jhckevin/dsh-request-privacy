...
import { DEFAULT_REQUEST_PRIVACY_CONFIG, REQUEST_PRIVACY_SETTINGS_NAMESPACE, resolveRequestPrivacyConfig } from './privacy.ts'
import type { RequestPrivacyConfig } from './privacy.ts'
...
export type { RequestPrivacyConfig, RequestPrivacyPreview } from './privacy.ts'
export { DEFAULT_REQUEST_PRIVACY_CONFIG, REQUEST_PRIVACY_SETTINGS_NAMESPACE, privacyHeaders, requestPrivacyPreview } from './privacy.ts'

export const name = 'request-privacy'
export const inject = ['llm']

const NS = REQUEST_PRIVACY_SETTINGS_NAMESPACE
const DEFAULT_API_KEY_ENV = 'DEEPSEEK_API_KEY'
/** The single provider route this plugin owns. */
export const PROVIDER = 'deepseek-private'
...
export interface Config {
  /** Minimize request headers; disabling restores native headers on the next request. */
  enabled?: boolean
  /** Fixed neutral application identity. Arbitrary client impersonation is unsupported. */
  identity?: RequestPrivacyConfig['identity']
  /** Optional correlation fields remain omitted in this release. */
...
export const Config: z<Config> = z.object({
  enabled: z.boolean().default(DEFAULT_REQUEST_PRIVACY_CONFIG.enabled),
  identity: z.const('private-client').default(DEFAULT_REQUEST_PRIVACY_CONFIG.identity),
  omitCorrelationHeaders: z.const(true).default(true),
...
export function resolveAdapterOptions(config: Config, environment?: LaunchEnvironmentSnapshot): ResolvedDeepSeekOptions {
  ...
  return {
    privacy: resolveRequestPrivacyConfig(config),
...
export function apply(ctx: Context, config: Config): void {
  ...
  const adapter = new DeepSeekAdapter({ options, resolveApiKey, lifecycleSignal: lifecycle.signal })
  ctx.llm.registerConfigurableProviders([
    { provider: PROVIDER, displayName: 'Private DeepSeek', settingsNs: NS, settingsPath: [] },
  ])
  ...
  ctx.inject(['settings'], inner => inner.settings.installSection(ctx, NS, Config, config, {
    setSource: (source) => {
      current = source
    },
    onChange: ensureRegistrationFacts,
  }))
  // Register last so Cordis runs this disposer first: active transports are
  // cancelled before the provider directory, adapter route, and settings
  // effects unwind. The same path covers config disable, HMR, and removal.
  ctx.effect(() => () => {
    lifecycle.abort('request-privacy plugin unloaded')
  }, 'request-privacy: abort in-flight provider requests')
}
